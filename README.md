...
- Neither the image nor the video panel (`gr.HTML` iframe) has an explicit "clear" action - each stays showing whatever was last generated/loaded in that chat until a new one replaces it. A brand-new chat starts with neither, since `image_b64`/`video_id` don't exist in the checkpoint until something sets them.
